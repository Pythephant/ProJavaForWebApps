...
<body>
	<h2>Tic Tac Toe</h2>
	<span class="player-label">You:</span> ${username}
	<br />
						...
						function() {
							...
							try {
								server = new WebSocket(
										'ws://'
												...
												+ '<c:url value="/game/ticTacToe/${gameId}/${username}"><c:param name="action" value="${action}" /> </c:url>');
							} catch (error) {
							...
							server.onmessage = function(event) {
								...
								if (message.action == 'gameStarted') {
									...
									$(
											'#r' + message.move.row + 'c'
													+ message.move.column)
											.unbind('click')
											.removeClass('game-cell-selectable')
											.addClass(
													'game-cell-opponent game-cell-taken');
									toggleTurn(true);
							...
							var toggleTurn = function(isMyTurn, message) {
								...
								if (myTurn) {
									...
									status
											.text(message
													|| 'Waiting on your opponent to move.');
									$('.game-cell-selectable').removeClass(
							...
							move = function(row, column) {
								...
								if (server != null) {
									...
									$('#r' + row + 'c' + column)
											...
											.removeClass('game-cell-selectable')
											.addClass(
													'game-cell-player game-cell-taken');
									toggleTurn(false);
								} else {
									modalErrorBody
											.text('Not connected to came server.');
									modalError.modal('show');
								}
							};
						});
